...
2.  **Consent & Age Agreement Screen**:
    ...
    *   **Privacy Notice Gate**: The Heir opens the in-page Privacy Notice, reviews deployment-aware data handling language, and checks the mandatory privacy acknowledgment.
...
3.  **Authentication Handshake & Identity Verification Onboarding**:
    ...
    *   **Future Federated Login Linking**: If the deployment operator enables OIDC/SSO, the Heir may link Google, Apple, Facebook, or another provider only after Executor identity approval (`identity_verified = true`, status `'ACTIVE'`). Before approval, SSO buttons must not create durable login links. Once linked, future logins resolve the external identity to the already-verified local Heir account.
    *   **Setup Practice State**: If the session status is `'SETUP'`, real sliders and mediator inputs remain locked. Once the Executor publishes the session-specific fictional rehearsal, the dashboard presents **Practice the allocation process** as Step 1. The Heir experiences the photo catalog, item details, 1,000-point allocation, review/submit, waiting room, and explained result without using or storing real estate preferences.
    *   **Registered Completion**: Reaching the rehearsal result records `practice_completed_at` for the logged-in Heir. The Executor sees only completion status, never the fictional points.
    *   **Launch Transition**: New sessions require all registered participating Heirs to complete practice by default. The Admin may explicitly make practice optional for an exception. Once the launch gate passes and the Admin clicks **"Launch Real Allocation"**, a WebSocket event updates the Zustand store and unlocks the real sliders and chat for verified, `'ACTIVE'` heirs.
4.  **Return Visits & Heirs in Multiple Estates**: After onboarding, a Heir can return via `/login` with their email/username and password instead of re-using the (potentially expired) invite link.
    *   **Same person, multiple estates**: A Heir is not limited to one mediation session — the same email can legitimately be onboarded as an Heir in two or more separate estates (e.g. they are an heir to two different decedents administered by two different Executors), each with its own independently-set password.
    *   **Disambiguation at login**: If the submitted email/username and password both validly match Heir records in more than one session, `POST /api/auth/heir-login` does not guess which one to log into. It returns a list of matching sessions (by `title`), and the frontend renders a **Choose an Estate** picker. Selecting an estate resubmits the login with that session's ID, completing authentication into the correct, specific session.
    *   **Different password per estate**: Since each session-scoped Heir record has its own password, a Heir with two estates may also have two different passwords; only the session(s) whose password matches the one entered will appear as candidates.
    *   **Switching estates from inside the dashboard**: Once logged into one estate, the dashboard header shows a **Switch Estate** button (Heirs only). Clicking it calls `GET /api/auth/heir-sessions` to list every other session sharing the same email/username, and lets the Heir pick a different one without logging out or re-entering a password. Selecting a different estate calls `POST /api/auth/heir-switch-session`, which re-issues the JWT cookie scoped to the new session; the WebSocket connection, asset gallery, and valuations reload for the newly active estate.
...
1.  **Session Creation**: The Admin navigates to the Admin Panel and creates a new Estate Mediation Session.
    *   **Return Visits & Hard Refresh**: After Admin login, the HTTP-only cookie is the source of truth for the browser session. A hard refresh on `/admin` must call `GET /api/auth/me`, show a restoring state while the check is pending, restore the Admin console when `role == 'ADMIN'`, and reload the session list without forcing the Admin to sign in again. If the cookie is missing or expired, the normal login/setup gate is shown. A valid Heir cookie must not open the Admin console.
    *   **Growing session portfolio**: The Admin may eventually manage dozens or hundreds of estate sessions from a phone. The `/admin` landing view must therefore behave like a scalable session index: search by estate name/status/id, filter by lifecycle status, sort by date/title/status, switch between comfortable cards and compact list rows, and page or incrementally load results. The UI should show a small summary of totals/statuses while keeping each session row calm: one primary "open" target plus secondary edit/delete actions.
    *   **Logout**: Admin logout calls `POST /api/auth/logout`, clears the server cookie, clears local Admin session state, and removes any saved active Admin console session selection.
...
### Step 0.5: Optional Open-Source Federated Login Setup
1.  **Local Login First**: First-admin setup always creates a local Argon2 password credential. SSO is optional and cannot be the only way to recover a self-hosted deployment.
2.  **OIDC Provider Configuration**: The Admin may enable a generic OpenID Connect provider from Settings after local admin setup. Recommended open-source brokers are Keycloak or Authentik, which can in turn connect to Google, Apple, Facebook, Microsoft, GitHub, or enterprise IdPs.
3.  **Configuration Inputs**: Admin enters issuer URL, client ID, client secret, redirect URI, scopes, enabled flag, and optional email-domain allowlist. Secrets are encrypted at rest.
4.  **Admin Identity Linking**: Existing Admin must explicitly link their external identity after authenticating locally. A deployment must retain at least one usable Admin login method before local password login can be disabled.
5.  **Heir Identity Linking**: Heirs link an external identity only after Executor identity approval, from an authenticated dashboard/settings flow. The system keys identity records by `issuer + subject`, not by email alone. Invite acceptance and `PROFILE_HOLD` are intentionally excluded from durable SSO linking.
6.  **Operator Responsibility**: The UI must explain that social login availability depends on the deployment operator's configured OIDC provider and provider terms. The Estate Steward ships open-source OIDC support, not hardcoded vendor accounts.
