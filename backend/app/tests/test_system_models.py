...
def test_system_models_default_names():
    ...
    assert "Qwen3" in fast["name"]
    assert "Qwen3" in slow["name"] or "14B" in slow["name"]
    assert "Qwen3-VL" in vision["name"] or "qwen3-vl" in vision["name"].lower()
...
def test_system_models_kokoro_tts_unchanged_by_env_vars():
    ...
    os.environ["FAST_THINKER_MODEL"] = "qwen3:1.7b"
    os.environ["SLOW_THINKER_MODEL"] = "qwen3:14b"
    response = client.get("/api/system/models")
    assert response.status_code == 200
    models = response.json()["models"]
    tts = models[3]
